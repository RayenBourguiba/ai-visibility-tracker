...
const mentionRateB = Math.round((B.results.filter((r: Result) => r.isMentioned).length / totalB) * 100);
  ...
  const diffs = promptIds.map((pid) => {
    ...
    return {
      ...
      a: a
        ? { mentioned: a.isMentioned, position: a.position, score: a.scores?.visibility_score ?? 0 }
        : null,
      b: b
        ? { mentioned: b.isMentioned, position: b.position, score: b.scores?.visibility_score ?? 0 }
        : null,
      deltaScore: (b?.scores?.visibility_score ?? 0) - (a?.scores?.visibility_score ?? 0),
    };
  });
  ...
  const competitorsA = new Set(
    A.results.flatMap((r) => (Array.isArray(r.competitors) ? r.competitors : [])).map(String)
  );
  const competitorsB = new Set(
    B.results.flatMap((r) => (Array.isArray(r.competitors) ? r.competitors : [])).map(String)
  );

  const newCompetitors = Array.from(competitorsB).filter((c) => !competitorsA.has(c)).slice(0, 20);

  return NextResponse.json({
    runA: { id: A.id, engine: A.engine, createdAt: A.createdAt, score: scoreA, mentionRate: mentionRateA },
    runB: { id: B.id, engine: B.engine, createdAt: B.createdAt, score: scoreB, mentionRate: mentionRateB },
